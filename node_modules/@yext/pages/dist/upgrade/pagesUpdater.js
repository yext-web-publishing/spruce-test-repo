...
async function updatePackageDependency(packageName, version, install = false) {
  ...
  try {
    ...
    console.error(`Error updating ${packageName}: `, e.message);
    process.exit(1);
  }
}
async function removePackageDependency(packageName) {
  const packagePath = path.resolve("package.json");
  if (!fs.existsSync(packagePath)) {
    console.error(
  ...
  try {
    ...
    if (packageJson[dependencyType][packageName]) {
      ...
      packageJson[dependencyType][packageName] = void 0;
      fs.writeFileSync(packagePath, JSON.stringify(packageJson, null, 2));
      return true;
    }
  } catch (e) {
    console.error(`Error removing ${packageName}: `, e.message);
    process.exit(1);
  }
  return false;
}
function processDirectoryRecursively(currentPath, operation) {
...
const updateToUsePagesComponents = async (source) => {
  ...
  await updatePackageDependency(
    ...
    null,
    hasPagesSlashComponentsImports || hasSitesComponentsImports || hasReactComponentsImports || movedTsxMapsImports
  );
};
const updatePagesJSToCurrentVersion = async () => {
  ...
  try {
    ...
    console.error("Failed to upgrade pages version: ", e.message);
    process.exit(1);
  }
};
const updateDevDependencies = async () => {
...
const checkLegacyMarkdown = (source) => {
...
const replacePagesSlashComponentsImports = (source) => {
...
const replaceSitesComponentsImports = (source) => {
  let hasReplaced = false;
  const operation = async (filePath) => {
    const fileContent = fs.readFileSync(filePath, "utf8");
...
const replaceReactComponentsImports = (source) => {
  let hasReplaced = false;
  const operation = async (filePath) => {
    const fileContent = fs.readFileSync(filePath, "utf8");
...
const moveTsxMapsImportsToPagesComponents = (source) => {
  let updated = false;
  ...
  for (let i = 0; i < sourceFiles.length; i++) {
    ...
    for (let j = 0; j < importDeclarations.length; j++) {
      const importDeclaration = importDeclarations[j];
      if (importDeclaration.getModuleSpecifierValue() !== "@yext/components-tsx-maps" && importDeclaration.getModuleSpecifierValue() !== "@yext/components-tsx-geo") {
        continue;
      }
...
const removeFetchImport = (source) => {
...
const updatePackageEngines = () => {
  ...
  try {
    ...
    console.error("Error updating package engines: ", e.message);
  }
};
...
const updateServerlessFunctionTypeReferences = (serverlessFunctionsPath) => {
  let updated = false;
  const project = new Project({
    compilerOptions: {
      jsx: typescript.JsxEmit.ReactJSX,
...
export {
  ...
  removeFetchImport,
  replacePagesSlashComponentsImports,
  replaceReactComponentsImports,
