...
`;
const discoverInputs = async (templatePaths, redirectPaths, projectStructure) => {
  const entryPoints = {};
  const updateEntryPoints = async (dir, isRedirect) => (await readdir(dir, { withFileTypes: true })).filter((dirent) => !dirent.isDirectory()).map((file) => file.name).filter(
    (f) => f !== "_client17.tsx" && f !== "_client.tsx" && f !== "_server.tsx"
  ).forEach((template) => {
    const parsedPath = parse(template);
    const bundlePath = isRedirect ? projectStructure.config.subfolders.redirectBundle : template.includes(".client") ? projectStructure.config.subfolders.clientBundle : projectStructure.config.subfolders.serverBundle;
