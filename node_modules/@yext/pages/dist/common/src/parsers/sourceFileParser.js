...
class SourceFileParser {
   ...
   */
  getVariablePropertyByName(name, property) {
    ...
    for (const declaration of variableDeclarations) {
      if (declaration.getName() === name && declaration.isExported()) {
        ...
        const variableProperty = objectLiteral.getProperties().find(
          (prop) => prop.getName() === property
        );
        if (variableProperty) {
  ...
  getVariableDeclarationByType(type) {
    const declaration = this.sourceFile.getVariableDeclaration(
      (v) => v.getType().getText() === type
    ) ?? this.sourceFile.getVariableDeclaration(
      (v) => v.getType().getText().endsWith("." + type)
    );
  ...
  insertStatement(code, index = 0) {
    this.sourceFile.insertText(index, code);
    return this.getEndPos();
  }
  removeStatement(startIndex, endIndex) {
