...
const parse = (filepath, adjustForFingerprintedAsset) => {
  ...
  return {
    base,
    // the root file with extension
    name
    // the root file without extension
  };
};
const isStaticTemplateConfig = (streamId, streamConfig) => {
...
const convertStreamToStreamInternal = (stream) => {
  if (!stream)
    return;
