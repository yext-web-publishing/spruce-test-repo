...
const generateTestData = async (scope) => {
...
const generateTestDataForSlug = async (stdout, vite, slug, projectStructure) => {
  ...
  args.push("--slug", `"${slug}"`);
  const slugFields = /* @__PURE__ */ new Set();
...
const getSiteStream = (projectStructure) => {
...
const getDocumentBySlug = (parsedData, slug, slugFields, templateModuleCollection) => {
  if (!Array.isArray(parsedData)) {
    return parsedData;
  }
  ...
  const matchingSlugFieldsSet = /* @__PURE__ */ new Set();
  ...
  if (matchingTemplateModules.length === 0) {
    ...
    const templateNames = matchingTemplateModules.map((templateModule) => templateModule.config.name).join(", ");
    throw new Error(
